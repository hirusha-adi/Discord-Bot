...
owner_id = botconfigdata["ownerid"]
...
async def megaspamlol(ctx, *, number_of_times_spam_secret):
  if ctx.author.id == owner_id:
    ...
    embed=discord.Embed(title="NO PERMISSIONS", color=0xff0000)
    embed.add_field(name="", value="You have no permission to use this command!", inline=True)
    await ctx.send(embed=embed)
