...
class Main(commands.Cog, description="For bot information"):
    def __init__(self, client: commands.Bot):
        ...
        self.bot_current_version = getbase.BotBase.BOT_VERSION
        self.bot_creator_name = getbase.BotBase.CREATOR_NAME
    ...
    async def on_command_error(self, ctx, error):
        ...
        if isinstance(error, commands.MissingRequiredArgument):
            embed = discord.Embed(title=getembed.ErrorEmbeds.ARGS_TITLE,
                                  description=getembed.ErrorEmbeds.ARGS_DESCRIPTION, color=getembed.ErrorEmbeds.ARGS_COLOR)
            embed.set_author(name=getembed.ErrorEmbeds.ARGS_AUTHOR_NAME,
                             icon_url=getembed.ErrorEmbeds.ARGS_AUTHOR_LINK)
            embed.set_thumbnail(url=getembed.ErrorEmbeds.ARGS_THUMBNAIL)
            embed.add_field(name=getembed.ErrorEmbeds.ARGS_ERROR,
                            value=getembed.ErrorEmbeds.ARGS_ERROR_VALUE, inline=True)
            embed.add_field(name=getembed.ErrorEmbeds.ARGS_POSSIBLE_FIX,
                            value=getembed.ErrorEmbeds.ARGS_POSSIBLE_FIX, inline=True)
            await ctx.send(embed=embed)
            return
    ...
    async def status(self, ctx):
        ...
        try:
            ...
            embed = discord.Embed(color=getembed.Common.COLOR)
            embed.add_field(
                name="Announcements", value=getbase.BotBase.STATUS_ANNOUNCEMENTS, inline=False)
            embed.add_field(
                name="Servers", value=f"{len(self.client.guilds)}", inline=True)
            embed.add_field(name="Uptime", value=f"{text}", inline=True)
            embed.add_field(
                name="Version", value=f"{self.bot_current_version}", inline=True)
            embed.add_field(name="Source Code",
                            value=getbase.BotBase.SOURCE_CODE_LINK, inline=True)
            embed.add_field(
                name="Creator", value=f"{self.bot_creator_name}", inline=True)
            embed.add_field(
                name="Errors", value=getbase.BotBase.STATUS_ERRORS, inline=False)
            embed.set_footer(text=f"Requested by {ctx.author.name}")
            await loading_message.delete()
            await ctx.send(embed=embed)
