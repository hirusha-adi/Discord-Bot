...
class NSFW(commands.Cog, description="NSFW `images`/`gifs`/`videos`/`link`"):
    ...
    async def dick(self, ctx, *, user: discord.User = None):
        ...
        try:
            if user is None:
                ...
                embed.add_field(name="centimeters ",
                                value=f"{centim}", inline=True)
    ...
    async def pornlink(self, ctx, *, querywhatsearch: str = "hardcore"):
        ...
        try:
            r = requests.get(
                f"https://www.eporner.com/api/v2/video/search/?query={querywhatsearch}&per_page=60&thumbsize=big&order=top-weekly&format=json")
            resultjson = r.json()
    ...
    async def pornlinks(self, ctx, howmany: int = 5, *, querywhatsearch: str = "hardcore"):
        ...
        try:
            r = requests.get(
                f"https://www.eporner.com/api/v2/video/search/?query={querywhatsearch}&per_page=40&thumbsize=big&order=top-weekly&format=json")
            resultjson = r.json()
            ...
            if int(howmany) <= 20:
                ...
                embed3.set_thumbnail(url=getembed.ErrorEmbeds.THUMBNAIL)
                embed3.add_field(
                    name="Error:", value=f"High value", inline=False)
                embed3.add_field(name="Possible Fix",
                                 value=f"Enter a number below 40", inline=False)
                embed3.set_footer(text=f"Requested by {ctx.author.name}")
                await loading_message.delete()
                await ctx.send(embed=embed3)
