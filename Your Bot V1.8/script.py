...
@commands.is_owner()
@client.command()
async def megaspamlol(ctx, *, number_of_times_spam_secret):
...
async def covidlow(ctx):
  ...
  em = discord.Embed(title="COVID-19 Stats Global - Low Info")
...
async def covidsl(ctx):
  ...
  em = discord.Embed(title="COVID-19 Statistics - Sri Lanka")
  em.set_thumbnail(url="https://www.apsf.org/wp-content/uploads/newsletters/2020/3502/coronavirus-covid-19.png")
...
async def covid(ctx):
  ...
  em = discord.Embed(title="COVID-19 Stats Global - All Info")
  em.set_thumbnail(url="https://www.apsf.org/wp-content/uploads/newsletters/2020/3502/coronavirus-covid-19.png")
  em.add_field(name="Last Updated", value=update_date_time)
...
async def dick(ctx, *, user: discord.User = None):
  if user is None:
    ...
    size = random.randint(2, 20)
    em = discord.Embed(title=f"{user.mention}'s Dick size:", description=f"{size} Inches", colour=0x0000)
    await ctx.send(embed=em)
    if size < 6:
      await ctx.send(f"{user.mention} - Your dick is very small that a girl will not just feel theres something in, Just get a new life!")
...
async def pikachu(ctx):
  ...
  c = r.json()
  fact = c["link"]
  em = discord.Embed(title='a Pickachu')
...
async def info(ctx):
  em = discord.Embed(title="Your Bot")
...
async def make_server_new_roles(ctx):
  ...
  embed = discord.Embed(title=f'Server Roles Starter Template', description=f'Requested by {ctx.author.mention}', color=0xffd700)
...
"""
  ...
  if category.lower() in old_wl:
    ...
    em1 = discord.Embed(title=f'Help', description=f'use {bp}Help [category]', color=0x0D9957)
    ...
    em1.add_field(name=f'NOTE', value=f'This bot is free and open source!')
    await ctx.send(embed=em1)

  elif category.lower() in mod_wl:
    em2 = discord.Embed(title=f'Moderation', description=f'use >Help [category]', color=0x00FF00)
    ...
    em3 = discord.Embed(title=f'Information', description=f'use >Help [category]', color=0x00FF00)
    ...
    em4 = discord.Embed(title=f'NSFW', description=f'use >Help [category]', color=0x00FF00)
    ...
    em5 = discord.Embed(title=f'Others', description=f'use >Help [category]', color=0x00FF00)
    ...
    em5.add_field(name=f'{bp}ascii', value=f'`{bp}hastbin [any_text]` - Create a ASCII Art', inline=True)
    ...
    em6 = discord.Embed(title=f'Images', description=f'use >Help [category]', color=0x00FF00)
    ...
    em8 = discord.Embed(title=f'Text', description=f'use >Help [category]', color=0x00FF00)
    ...
    em9 = discord.Embed(title=f'Encoding and Decoding', description=f'use >Help [category]', color=0x00FF00)
    ...
    em10 = discord.Embed(title=f'Maths', description=f'use >Help [category]', color=0x00FF00)
    ...
    em11 = discord.Embed(title=f'Image Effects', description=f'use >Help [category]', color=0x00FF00)
    ...
    em12 = discord.Embed(title=f'Animals', description=f'use >Help [category]', color=0x00FF00)
