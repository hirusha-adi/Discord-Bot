        ...
        if ctx.author.id == self.bot_creator_id:
            try:
                if member == None:
                    ...
                    embed2.set_author(name=getembed.Common.AUTHOR, icon_url=getembed.Common.AUTHOR_LINK)
                    embed2.set_thumbnail(url=getembed.ErrorEmbeds.THUMBNAIL)
                    embed2.add_field(name="Error:", value=f"No user is mentioned", inline=False)
                    embed2.add_field(name="Possible Fix:", value=f"Enter the member-id or tag the member at the end of the message", inline=False)
                    embed2.set_footer(text=f"Requested by {ctx.author.name}")
                    await loading_message.delete()
                    await ctx.send(embed=embed2)
                    return
